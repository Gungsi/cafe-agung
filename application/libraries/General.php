...
class General {
  ...
  public function session_check(){
    ...
    $CI->user_level  = $CI->session->userdata('level');
    $CI->user_email  = $CI->session->userdata('email');
    $CI->user_hp     = $CI->session->userdata('no_hp');
    $CI->ip_address  = $this->get_ip();
    $CI->get_os      = $this->get_os();
    $CI->get_browser = $this->get_browser();
		$CI->get_device  = $this->get_device();

		$uri = $CI->uri->segment(1);

		if($uri=="dashboard" || $uri=="jenis" || $uri=="menu" || $uri=="makanan" || $uri=="diskon" || $uri=="transaksi"){
			if ($CI->user_id=="") {
